# XModem Developer Guide

This document provides a guide for developers looking to understand, extend, or adapt the `luci-app-xmodem` application.
...
The frontend communicates with the backend via API calls handled by `controller/xmodem.lua`. The primary endpoint is `/cgi-bin/luci/admin/modem/xmodem`. The action is determined by a `json` parameter in the request.
...
3.  **Backend Script**: The `xmodem.lua` controller executes a shell script (`/usr/share/xmodem/scan_modem.sh` or similar).
...
6.  **UCI Update**: The script updates the `xmodem` UCI configuration file with the information about the detected modems, creating a new "slot" for each.
...
Adapting `xmodem` for a new, unsupported modem generally involves the following steps:
...
By following these steps, you can integrate new modems into the `xmodem` ecosystem and take advantage of its management features.

## 5. Core Scripts in `/usr/share/xmodem`
...
-   **Commands & Parameters**:
    ...
    -   `add <slot>`: Adds or updates a modem configuration in UCI based on a bus slot ID (e.g., `1-1.2` for USB, `0000:01:00.0` for PCIe). It gathers device info and creates the `xmodem.<slot_name>` configuration section.
...
Adapting a new modem involves teaching `xmodem` about its characteristics and commands.
...
1.  **Create Vendor Script**:
    -   Create a new file: `/usr/share/xmodem/vendor/<new_vendor>.sh`.
    -   The best practice is to copy `/usr/share/xmodem/vendor/generic.sh` as a template. This ensures you have all the required function stubs.
...
```sh
# In /usr/share/xmodem/vendor/example_vendor.sh
...
```sh
# In /usr/share/xmodem/vendor/quectel.sh
