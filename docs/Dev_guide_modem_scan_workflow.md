...
在以下几个地方会触发模组扫描
负责扫描的都是 ```luci/luci-app-xmodem/root/usr/share/xmodem/modem_scan.sh``` 脚本
### 模组扫描流程：
...
*监控预设的USB端口* 遍历 uci 配置 ```xmodem.@modem-slot``` , 扫描 ```slot_type``` 为 ```usb``` 的设备，如果存在且含有加载了串口驱动的接口，尝试添加该设备
*监控预设的PCIe端口* 遍历 uci 配置 ```xmodem.@modem-slot``` , 扫描 ```slot_type``` 为 ```pcie``` 的设备，如果存在且含有加载了串口驱动的接口，尝试添加该设备
