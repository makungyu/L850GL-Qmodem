...
return view.extend({
	load: function() {
		return Promise.all([
			uci.load('xmodem'),
			xmodem.rcStatus('xmodem_network')
			...
			var statusPromises = sections.map(function(section) {
				return xmodem.getConnectStatus(section['.name'])
	...
	render: function() {
		...
		// XModem network RC status and control
		var rcStatus = s.option(form.DummyValue, '_rc_status', _('XModem Network'));
		rcStatus.rawhtml = true;
		rcStatus.cfgvalue = L.bind(function() {
			var status = this.rcStatusData && this.rcStatusData.xmodem_network ? this.rcStatusData.xmodem_network : null;
			...
			if (!running) {
				buttons.push(
					E('button', {
						...
						'click': ui.createHandlerFn(this, function() {
							var self = this;
							return callInitAction('xmodem_network', 'start')
								.then(function() {
									ui.addNotification(null, E('p', _('XModem Network started successfully')));
									return self.updateRcStatus();
								})
								.catch(function(err) {
									ui.addNotification(null, E('p', _('Failed to start XModem Network: ') + err.message), 'error');
									return self.updateRcStatus();
								});
						})
					}, _('Start'))
				...
				buttons.push(
					E('button', {
						...
						'click': ui.createHandlerFn(this, function() {
							var self = this;
							return callInitAction('xmodem_network', 'stop')
								.then(function() {
									ui.addNotification(null, E('p', _('XModem Network stopped successfully')));
									return self.updateRcStatus();
								})
								.catch(function(err) {
									ui.addNotification(null, E('p', _('Failed to stop XModem Network: ') + err.message), 'error');
									return self.updateRcStatus();
								});
						})
					}, _('Stop'))
				...
				buttons.push(
					E('button', {
						...
						'click': ui.createHandlerFn(this, function() {
							var self = this;
							return callInitAction('xmodem_network', 'restart')
								.then(function() {
									ui.addNotification(null, E('p', _('XModem Network restarted successfully')));
									return self.updateRcStatus();
								})
								.catch(function(err) {
									ui.addNotification(null, E('p', _('Failed to restart XModem Network: ') + err.message), 'error');
									return self.updateRcStatus();
								});
						})
					}, _('Restart'))
			...
			if (!enabled) {
				buttons.push(
					E('button', {
						...
						'click': ui.createHandlerFn(this, function() {
							var self = this;
							return callInitAction('xmodem_network', 'enable')
								.then(function() {
									ui.addNotification(null, E('p', _('XModem Network enabled successfully')));
									return self.updateRcStatus();
								})
								.catch(function(err) {
									ui.addNotification(null, E('p', _('Failed to enable XModem Network: ') + err.message), 'error');
									return self.updateRcStatus();
								});
						})
					}, _('Enable'))
				...
				buttons.push(
					E('button', {
						...
						'click': ui.createHandlerFn(this, function() {
							var self = this;
							return callInitAction('xmodem_network', 'disable')
								.then(function() {
									ui.addNotification(null, E('p', _('XModem Network disabled successfully')));
									return self.updateRcStatus();
								})
								.catch(function(err) {
									ui.addNotification(null, E('p', _('Failed to disable XModem Network: ') + err.message), 'error');
									return self.updateRcStatus();
								});
						})
					}, _('Disable'))
	...
	s.modaltitle = L.bind(function(section_id) {
		var name = uci.get('xmodem', section_id, 'name');
		var alias = uci.get('xmodem', section_id, 'alias');
	...
	o = s.option(form.Flag, 'enable_dial', _('Enable Dial'));
		...
		o.cfgvalue = function(section_id) {
			return uci.get('xmodem', section_id, 'alias') || '-';
		};

	o = s.option(form.DummyValue, 'state', _('Status'));
	o.cfgvalue = function(section_id) {
		var state = uci.get('xmodem', section_id, 'state');
	...
	o.editable = true;
	o.cfgvalue = L.bind(function(section_id) {
		var enable_dial = uci.get('xmodem', section_id, 'enable_dial');
		if (enable_dial === '0') {
			return E('span', {}, '-');
		}
		...
		if (isRunning) {
			...
			buttons.push(
				E('button', {
					'class': 'cbi-button cbi-button-reset',
					'click': ui.createHandlerFn(this, function(section_id) {
						return xmodem.modemHang(section_id).then(L.bind(function(result) {
				...
				E('button', {
					'class': 'cbi-button cbi-button-apply',
					'click': ui.createHandlerFn(this, function(section_id) {
						return xmodem.modemRedial(section_id).then(function(result) {
							if (result && result.result && result.result.status === '1') {
			...
			buttons.push(
				E('button', {
					'class': 'cbi-button cbi-button-action',
					'click': ui.createHandlerFn(this, function(section_id) {
						return xmodem.modemDial(section_id).then(L.bind(function(result) {
							if (result && result.result && result.result.status === '1') {
	...
	updateDialControlsForSection: function(section_id) {
		return xmodem.getDialStatus(section_id).then(L.bind(function(result) {
	...
	updateAllDialControls: function() {
		...
		sections.forEach(L.bind(function(section) {
			var enable_dial = uci.get('xmodem', section['.name'], 'enable_dial');
			if (enable_dial !== '0') {
	...
	updateStatusIndicator: function(section_id) {
		...
		return xmodem.getConnectStatus(section_id).then(L.bind(function(result) {
			// Update cached data
	...
	updateRcStatus: function() {
		...
		return xmodem.rcStatus('xmodem_network').then(L.bind(function(result) {
			this.rcStatusData = result;
			var status = result && result.xmodem_network ? result.xmodem_network : null;
	...
	showDialLogDialog: function(section_id) {
		...
		// Load and auto-refresh log content while the dialog is open
		this.loadDialLog(section_id, logContent);
		var refreshTimer = window.setInterval(L.bind(function() {
			if (!document.body.contains(logContent)) {
				window.clearInterval(refreshTimer);
				return;
			}
			this.loadDialLog(section_id, logContent, true);
		}, this), 3000);
	},

	formatDialLog: function(log) {
		if (!log) return '';
		...
		return log.split('\n').map(function(line) {
			...
			if ((m = message.match(/^modem_path=([^,]+),driver=([^,]+),interface=([^,]+),at_port=([^,]+),using_sim_slot:([^,]+),dns_list:([^,]*),bridge_enabled:([^,]+),bridge_port:(.*)$/))) {
				readable = 'Preparing modem connection: driver=' + m[2].toUpperCase() + ', interface=' + m[3] + ', AT port=' + m[4] + ', SIM slot=' + m[5] + '.';
			} else if ((m = message.match(/^info sim card is ready/))) {
				readable = 'SIM card is ready.';
			} else if ((m = message.match(/^info sim card is miss/))) {
				readable = 'SIM card is not detected yet.';
			} else if ((m = message.match(/^create interface ([^ ]+) with proto ([^ ]+) and metric (.+)$/))) {
				readable = 'Created network interface ' + m[1] + ' using ' + m[2].toUpperCase() + ' protocol, metric ' + m[3] + '.';
			} else if ((m = message.match(/^create interface ([^ ]+)v6 with proto ([^ ]+) and metric (.+)$/))) {
				readable = 'Created IPv6 companion interface ' + m[1] + 'v6 using ' + m[2].toUpperCase() + ', metric ' + m[3] + '.';
			} else if ((m = message.match(/^interface ([^ ]+) already set to (.+)$/))) {
				readable = 'Network interface ' + m[1] + ' is already bound to ' + m[2] + '.';
			} else if ((m = message.match(/^set interface ([^ ]+) to (.+)$/))) {
				readable = 'Bound network interface ' + m[1] + ' to device ' + m[2] + '.';
			} else if (message === 'network reload') {
				readable = 'Reloaded OpenWrt network service.';
			} else if (message === 'firewall reload') {
				readable = 'Reloaded firewall rules.';
			} else if ((m = message.match(/^dialing (.+) driver (.+)$/))) {
				readable = 'Starting modem dial process using ' + m[2].toUpperCase() + ' mode.';
			} else if ((m = message.match(/^dialing: vendor:([^;]+); platform:([^;]+); driver:([^;]+); apn:([^;]*); command:(.+) pdp_index:(.+)$/))) {
				readable = 'Sending dial command for ' + m[1] + ' ' + m[2] + ' modem in ' + m[3].toUpperCase() + ' mode: ' + m[5] + ' (PDP ' + m[6] + ').';
			} else if ((m = message.match(/^dial_cmd: (.*); cgdcont_cmd: (.*); ppp_auth_cmd:(.*)$/))) {
				readable = 'Dial profile prepared: data command=' + m[1] + ', PDP context=' + m[2] + '.';
			} else if ((m = message.match(/^ip changed from (.*) to (.*)$/))) {
				readable = 'Network address changed from [' + (m[1] || 'none') + '] to [' + m[2] + '].';
			} else if ((m = message.match(/^ip_change_l850 .*force=([^ ]+) refresh=(.+)$/))) {
				readable = 'Applying L850GL NCM IP refresh policy: force=' + m[1] + ', mode=' + m[2] + '.';
			} else if ((m = message.match(/^ncm fast re-ip: (.+)$/))) {
				readable = 'Fast NCM recovery selected: ' + m[1] + '.';
			} else if ((m = message.match(/^set ([^ ]+) to ([^ ]+) gw ([^ ]+) \(force=([^,]+), refresh=([^\)]+)\)$/))) {
				readable = 'Applied network settings on ' + m[1] + ': IP ' + m[2] + ', gateway ' + m[3] + ', refresh=' + m[5] + '.';
			} else if ((m = message.match(/^hang up (.+) driver (.+)$/))) {
				readable = 'Stopping modem session for ' + m[2].toUpperCase() + ' mode.';
			} else if (message === 'ecm_hang') {
				readable = 'Stopping NCM/ECM data session.';
			} else if (message === 'auto_dial_hang') {
				readable = 'Stopping modem auto-dial session.';
			} else if ((m = message.match(/^delete interface (.+)$/))) {
				readable = 'Removed network interface ' + m[1] + '.';
			} else if ((m = message.match(/^at port response unexpected(.*)$/))) {
				readable = 'AT port returned an unexpected response' + (m[1] ? ': ' + m[1].trim() : '.') ;
			} else if ((m = message.match(/^unexpected AT response count=(\d+), fast redial$/))) {
				readable = 'Unexpected AT response repeated ' + m[1] + ' time(s); starting fast redial.';
			}
	...
	loadDialLog: function(section_id, container, silent) {
		return xmodem.getDialLog(section_id).then(L.bind(function(result) {
			while (container.firstChild) {
				container.removeChild(container.firstChild);
			}
			...
			if (result && result.log) {
				if (result.log.trim() === '') {
					...
					var formattedLog = this.formatDialLog(result.log);
					var textarea = E('textarea', { 'class': 'dial-log-content','readonly':'readonly','rows':'20' }, formattedLog);
	...
	downloadDialLog: function(section_id) {
		return xmodem.getDialLog(section_id).then(function(result) {
			if (result && result.log) {
				var name = uci.get('xmodem', section_id, 'name') || section_id;
				var alias = uci.get('xmodem', section_id, 'alias') || '';
		...
		ui.showModal(_('Clear Dial Log'), [
			...
			E('div', { 'class': 'right' }, [
				...
				E('button', {
					'class': 'cbi-button cbi-button-negative',
					'click': L.bind(function() {
						xmodem.clearDialLog(section_id).then(L.bind(function(result) {
							if (result && result.result && result.result.status === '1') {
	...
	handleSaveApply: function(ev, mode) {
		return this.handleSave(ev).then(function() {
			return callInitAction('xmodem_network', 'reload');
		});
	},
