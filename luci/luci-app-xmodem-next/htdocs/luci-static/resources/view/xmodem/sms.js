...
'require dom';
'require uci';
'require xmodem.sms as smsService';
...
return view.extend({
	...
	getUtcOffsetSetting: function(type) {
		...
		var key = 'xmodem_sms_utc_offset_' + type;
	...
	renderConversationsList: function(contentArea, conversations, configSection) {
		...
		for (var i = 0; i < conversations.length; i++) {
			...
			row.addEventListener('click', L.bind(function(contact) {
				...
				window.location.href = L.url('admin/modem/xmodem/sms/conversation', encodeURIComponent(contact));
			}, this, conv.contact));
					...
					'click': function() {
						...
						smsService.sendSms(configSection, phone, message, encoding).then(function(result) {
							...
							if (result.success) {
								...
								setTimeout(function() {
									window.location.href = L.url('admin/modem/xmodem/sms/conversation', encodeURIComponent(phone));
								}, 500);
							}
						}).catch(function(err) {
							ui.hideModal();
...
loadUciConfig: function(configSection, smsDbPathInput, smsAutoDeleteCheckbox) {
	return uci.load('xmodem').then(function() {
		// Load sms_db_path
		var dbPath = uci.get('xmodem', configSection, 'sms_db_path');
		if (dbPath) {
		...
		var autoDelete = uci.get('xmodem', configSection, 'sms_auto_delete_from_sim');
...
saveUciOption: function(configSection, option, value) {
	return uci.load('xmodem').then(function() {
		uci.set('xmodem', configSection, option, value);
		return uci.save();
	}).then(function() {
