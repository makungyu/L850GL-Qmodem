# luci-app-lpac — eSIM Profile Manager for OpenWrt

## What is this?

**luci-app-lpac** is a LuCI web interface and CLI tool for managing eSIM profiles on OpenWrt routers via **lpac 2.3.0**.

Adapted for **OpenWrt 24.10.x / 25.x** with three APDU backends:

- **MBIM** — primary, for modems in MBIM mode (recommended for XModem)
...
This package is the official eSIM web UI for XModem. When installed with XModem:

- Appears as a tab inside XModem: **Modem → XModem → eSIM Manager**
...
When using XModem, enable eSIM support in `make menuconfig`:

```
Utilities → xmodem → [*] Add eSIM/eUICC management (lpac) SUPPORT
```

This automatically installs `lpac`, `luci-app-lpac`, and `xmodem-esim-bot`.
...
apk add --allow-untrusted /tmp/luci-app-lpac.apk

# OpenWrt 24.x (opkg)
opkg install /tmp/lpac_2.3.0.ipk
opkg install /tmp/luci-app-lpac.ipk
```
...
For XModem users, this works out-of-box. The web UI Configuration tab allows changing all settings.
...
- [afadillo-a11y/luci-app-lpac](https://github.com/afadillo-a11y/luci-app-lpac) — upstream fork
- [FUjr/XModem](https://github.com/FUjr/XModem) — XModem modem management
