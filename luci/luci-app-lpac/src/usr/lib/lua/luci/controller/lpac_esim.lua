...
function index()
    -- Main page entry - nested under XModem as sub-menu
    entry({"admin", "modem", "xmodem", "esim"}, firstchild(), _("eSIM"), 10)
    entry({"admin", "modem", "xmodem", "esim", "info"}, template("lpac_esim/main"), _("Info"), 1)
    entry({"admin", "modem", "xmodem", "esim", "profile_list"}, template("lpac_esim/profiles_page"), _("Profiles"), 2)
    entry({"admin", "modem", "xmodem", "esim", "download_profile"}, template("lpac_esim/download_page"), _("Downloads"), 3)
    entry({"admin", "modem", "xmodem", "esim", "notifications"}, template("lpac_esim/notifications_page"), _("Notif"), 4)
    entry({"admin", "modem", "xmodem", "esim", "configuration"}, template("lpac_esim/config_page"), _("Config"), 5)
    entry({"admin", "modem", "xmodem", "esim", "bot"}, template("lpac_esim/telegram_page"), _("TgBot"), 6)

    -- Read-only endpoints (GET)
    entry({"admin", "modem", "xmodem", "esim", "profiles"},     call("api_profiles"),     nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "chip"},         call("api_chip"),         nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "modem_status"}, call("api_modem_status"), nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "notif_list"},   call("api_notif_list"),   nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "lock_status"},  call("api_lock_status"),  nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "config"},       call("api_get_config"),   nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "connectivity"}, call("api_connectivity"), nil).leaf = true

    -- Write endpoints (POST, some async)
    entry({"admin", "modem", "xmodem", "esim", "switch"},       call("api_switch"),       nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "reboot_modem"}, call("api_reboot"),       nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "notif_clear"},  call("api_notif_clear"),  nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "save_config"},  call("api_save_config"),  nil).leaf = true

    -- Download / Delete / Nickname / Notifications
    entry({"admin", "modem", "xmodem", "esim", "download"},      call("api_download"),      nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "delete"},        call("api_delete"),        nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "nickname"},      call("api_nickname"),      nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "notif_process"}, call("api_notif_process"), nil).leaf = true

    -- Component version endpoint
    entry({"admin", "modem", "xmodem", "esim", "version"},    call("api_version"),    nil).leaf = true

    -- Telegram Bot endpoints
    entry({"admin", "modem", "xmodem", "esim", "telegram_config"},      call("api_telegram_config"),      nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "telegram_status"},      call("api_telegram_status"),      nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "save_telegram_config"}, call("api_save_telegram_config"), nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "test_telegram"},        call("api_test_telegram"),        nil).leaf = true
    entry({"admin", "modem", "xmodem", "esim", "telegram_toggle"},      call("api_telegram_toggle"),      nil).leaf = true
end
...
function exec_script(cmd, timeout, silent)
    ...
    -- Cap run.log so frequent polling can't fill tmpfs (truncate if > 256 KB)
    sys.exec(string.format("[ -f %s ] && [ \"$(wc -c < %s 2>/dev/null)\" -gt 262144 ] && : > %s; true", RUN_LOG, RUN_LOG, RUN_LOG))
...
function api_telegram_config()
    ...
    local config = {}
    tg_uci:foreach("xmodem_esim_bot", "bot", function(s)
        config = s
    end)
...
function api_save_telegram_config()
    ...
    -- Save to UCI
    local tg_uci = require("luci.model.uci").cursor()
    tg_uci:set("xmodem_esim_bot", "main", "bot")
    tg_uci:set("xmodem_esim_bot", "main", "enabled", enabled)
    -- Only update token if not masked
    if token ~= "" and not token:match("^........%.%.%.") then
        tg_uci:set("xmodem_esim_bot", "main", "token", token)
    end
    tg_uci:set("xmodem_esim_bot", "main", "chat_id", chat_id)
    tg_uci:set("xmodem_esim_bot", "main", "poll_interval", poll_interval)
    tg_uci:set("xmodem_esim_bot", "main", "allow_disruptive", allow_disruptive)
    tg_uci:set("xmodem_esim_bot", "main", "require_confirm", require_confirm)

    if tg_uci:commit("xmodem_esim_bot") then
        sys.exec("/etc/init.d/xmodem_esim_bot reload 2>/dev/null")
...
function api_test_telegram()
    ...
    local cmd = string.format(
        "curl -s -m 10 -X POST 'https://api.telegram.org/bot%s/sendMessage' -d 'chat_id=%s' -d 'text=✅ XModem eSIM Bot - connection test OK!' 2>/dev/null",
        token, chat_id
    )
...
function api_telegram_toggle()
    ...
    if action == "start" then
        sys.exec("/etc/init.d/xmodem_esim_bot start 2>/dev/null")
    else
        sys.exec("/etc/init.d/xmodem_esim_bot stop 2>/dev/null")
    end
    send_json({ success = true })
end
