...
add_bar_info_entry()
{
    ...
    if [ -n "$entry_class" ]; then
        json_add_string "class" "$entry_class"
        json_add_string "class_origin" "$entry_class"
    fi
    if [ -n "$extra_info" ]; then
        json_add_string "extra_info" "$extra_info"
    fi
...
# Usage: set_4g_cell_info mcc mnc tac cell_id earfcn pci band ul_bw dl_bw rsrp rsrq sinr rssnr rxlev [signal_class]
set_4g_cell_info() {
    local mcc="$1"
    local mnc="$2"
    local tac="$3"
    ...
    add_plain_info_entry "UL Bandwidth" "$ul_bw" "UL Bandwidth"
    add_plain_info_entry "DL Bandwidth" "$dl_bw" "DL Bandwidth"
    add_bar_info_entry "RSRP" "$rsrp" "Reference Signal Received Power" -140 -44 dBm "$signal_class"
    add_bar_info_entry "RSRQ" "$rsrq" "Reference Signal Received Quality" -19.5 -3 dB "$signal_class"
    add_bar_info_entry "SINR" "$sinr" "Signal to Interference plus Noise Ratio" 0 30 dB "$signal_class"
    add_plain_info_entry "RSSNR" "$rssnr" "Radio Signal Strength Noise Ratio"
    add_bar_info_entry "RxLev" "$rxlev" "Received Signal Level" -120 -25 dBm "$signal_class"
