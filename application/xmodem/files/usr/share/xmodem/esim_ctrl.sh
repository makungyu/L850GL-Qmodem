#!/bin/sh
# esim_ctrl.sh - eSIM/eUICC management integration for XModem
...
source /usr/share/xmodem/modem_util.sh
...
get_mbim_device() {
    ...
    if [ -z "$wdm_name" ]; then
        local net_dev=$(uci -q get xmodem.$config_section.network)
...
ensure_mbim_proxy() {
    ...
    # xmodem_network at boot, so this is mostly a safety net for the
...
stop_dial_for_esim() {
    ...
    # Kill the quectel-CM process via xmodem_network
    /etc/init.d/xmodem_network hang "$config_section"
...
restart_dial_after_esim() {
    ...
    # Restart dial via xmodem_network
    /etc/init.d/xmodem_network dial "$config_section"
}

do_profile_enable() {
    local iccid=$(echo "$json_data" | jq -r '.iccid // empty' 2>/dev/null)
    local refresh=$(echo "$json_data" | jq -r '.refresh // "1"' 2>/dev/null)
