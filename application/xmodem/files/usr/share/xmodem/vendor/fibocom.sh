...
source /usr/share/xmodem/generic.sh
...
get_mode()
{
    ...
    available_modes=$(uci -q get xmodem.$config_section.modes)
...
add_intel_network_details()
{
    local ca_all ca sq csq rsrp_num rsrq_num snr_num rsrp rsrq sinr rssi
    ca_all=$(at $at_port "AT+GTCAINFO?" | grep "+GTCAINFO:" | sed 's/+GTCAINFO://g' | sed 's/\r//g' | awk -F',' 'NF>=10')
    ca=$(echo "$ca_all" | awk -F',' 'NF>=14' | head -n1)
    [ -z "$ca" ] && return

    sq=$(at $at_port "AT+XCESQ?" | grep "+XCESQ:" | sed 's/+XCESQ://g' | sed 's/\r//g' | head -n1)
    csq=$(at $at_port "AT+CSQ" | grep "+CSQ:" | sed 's/+CSQ://g' | awk -F',' '{print $1}' | tr -d ' \r')

    local dl_bw=$(_xmm_bw_mhz "$(echo "$ca" | awk -F',' '{print $13}' | tr -d ' ')")
    rsrq_num=$(echo "$sq" | awk -F',' '{print $6}' | tr -d ' ')
    rsrp_num=$(echo "$sq" | awk -F',' '{print $7}' | tr -d ' ')
    snr_num=$(echo "$sq" | awk -F',' '{print $8}' | tr -d ' ')
    ...
    local ca_count primary_band secondary_bands active_bands carrier_summary
    ca_count=$(echo "$ca_all" | wc -l | tr -d ' ')
    while IFS= read -r ca_line; do
        [ -z "$ca_line" ] && continue
        local ca_idx=$(echo "$ca_line" | awk -F',' '{print $1}' | tr -d ' ')
        local ca_band=$(echo "$ca_line" | awk -F',' '{print $2}' | tr -d ' ')
        local ca_earfcn=$(echo "$ca_line" | awk -F',' '{print $7}' | tr -d ' ')
        local ca_dl_earfcn=$(echo "$ca_line" | awk -F',' '{print $11}' | tr -d ' ')
        local ca_dl_bw=$(_xmm_bw_mhz "$(echo "$ca_line" | awk -F',' '{print $13}' | tr -d ' ')")
        [ -z "$ca_dl_earfcn" ] && ca_dl_earfcn="$ca_earfcn"
        [ -z "$ca_band" ] && continue
        [ -z "$primary_band" ] && primary_band="$ca_band"
        if [ "$ca_idx" = "1" ]; then
            primary_band="$ca_band"
        else
            [ -n "$secondary_bands" ] && secondary_bands="$secondary_bands, "
            secondary_bands="${secondary_bands}B${ca_band}"
        fi
        [ -n "$active_bands" ] && active_bands="$active_bands + "
        active_bands="${active_bands}B${ca_band}"
        [ -n "$carrier_summary" ] && carrier_summary="$carrier_summary\n"
        carrier_summary="${carrier_summary}CC${ca_idx}: B${ca_band}"
        [ -n "$ca_dl_earfcn" ] && carrier_summary="${carrier_summary} EARFCN ${ca_dl_earfcn}"
        [ -n "$ca_dl_bw" ] && carrier_summary="${carrier_summary} ${ca_dl_bw}MHz"
    done <<EOF
$ca_all
EOF

    class="Network Information"
    [ -n "$active_bands" ] && add_plain_info_entry "Active LTE Bands" "$active_bands" "Active LTE Bands"
    [ -n "$primary_band" ] && add_plain_info_entry "Primary LTE Band" "B${primary_band}" "Primary LTE Band"
    [ -n "$secondary_bands" ] && add_plain_info_entry "Secondary LTE Bands" "$secondary_bands" "Secondary LTE Bands"
    [ "$ca_count" -gt 0 ] 2>/dev/null && add_plain_info_entry "Active LTE Carriers" "$ca_count" "Active LTE Carriers"
    [ -n "$carrier_summary" ] && add_plain_info_entry "LTE CA Details" "$carrier_summary" "LTE Carrier Aggregation Details"
    add_bar_info_entry "RSRP" "$rsrp" "Reference Signal Received Power" -140 -44 dBm "Network Information"
    add_bar_info_entry "RSRQ" "$rsrq" "Reference Signal Received Quality" -19.5 -3 dB "Network Information"
    add_bar_info_entry "SINR" "$sinr" "Signal to Interference plus Noise Ratio" 0 30 dB "Network Information"

    local n_rb=""
    case "$dl_bw" in
        "1.4") n_rb=6 ;; "3") n_rb=15 ;; "5") n_rb=25 ;;
        "10") n_rb=50 ;; "15") n_rb=75 ;; "20") n_rb=100 ;;
    esac
    if [ -n "$rsrp" ] && [ -n "$rsrq" ] && [ -n "$n_rb" ]; then
        rssi=$(awk "BEGIN{printf \"%.0f\", $rsrp - ($rsrq) + 10*log($n_rb)/log(10)}")
    else
        case "$csq" in ''|99|*[!0-9]*) ;; *) rssi=$(( -113 + 2 * csq )) ;; esac
    fi
    [ -n "$rssi" ] && add_bar_info_entry "RSSI" "$rssi" "Received Signal Strength Indicator" -120 -20 dBm "Network Information"
}
...
network_info()
{
    ...
    [ "$platform" = "intel" ] && add_intel_network_details
    case $platform in
...
get_neighborcell_intel()
{
    ...
    band_l=$(echo "$lp" | grep '^band_info=' | head -1 | cut -d= -f2 | tr -d ' ')
    json_add_object "lockcell_status"
    case "$freq" in ""|0|65535) json_add_string "Status" "unlock" ;; *) json_add_string "Status" "lock" ;; esac
    ...
    [ -n "$band_l" ] && [ "$band_l" != "0" ] && [ "$band_l" != "255" ] && json_add_string "LTE BAND" "$band_l"
    json_add_string "SCS" ""
...
lockcell_intel()
{
    local b=255 p=65535 cur cur_pci lock_state
    [ -n "$band" ] && [ "$band" != "null" ] && [ "$band" != "0" ] && b="$band"
    [ -n "$pci" ] && [ "$pci" != "null" ] && [ "$pci" != "0" ] && p="$pci"
    if [ -z "$arfcn" ] || [ "$arfcn" = "null" ] || [ "$arfcn" = "0" ]; then
        lock_state=$(at $at_port 'at@nvm:dyn_cps.nas_asm.freq_lock_params.*??' | sed 's/\r//g')
        cur=$(echo "$lock_state" | grep '^frequency=' | head -1 | cut -d= -f2 | tr -d ' ')
        cur_pci=$(echo "$lock_state" | grep '^psc_or_pci=' | head -1 | cut -d= -f2 | tr -d ' ')
        [ -z "$cur" ] && cur=0
        [ -z "$cur_pci" ] && cur_pci=65535
        res=$(at $at_port "AT@SIC:FREQ_LOCK(0,3,255,0,$cur,$cur_pci)")
    else
...
set_neighborcell(){
    ...
    if echo "$res" | grep -q "OK"; then
        ...
        if [ -z "$arfcn" ] || [ "$arfcn" = "null" ] || [ "$arfcn" = "0" ]; then
            json_add_string "message" "Cell lock removed; modem is reconnecting"
        else
            json_add_string "message" "Cell lock applied; modem is reconnecting"
        fi
    else
        json_add_string "status" "0"
        json_add_string "message" "Cell lock command failed"
    fi
...
cell_info_intel()
{
    ...
    local ca_all=$(at $at_port "AT+GTCAINFO?" | grep "+GTCAINFO:" | sed 's/+GTCAINFO://g' | sed 's/\r//g' | awk -F',' 'NF>=10')
    local ca=$(echo "$ca_all" | awk -F',' 'NF>=14' | head -n1)
    if [ -z "$ca" ]; then
    ...
    local ca_count=$(echo "$ca_all" | wc -l | tr -d ' ')
    local primary_band=""
    local secondary_bands=""
    local active_bands=""
    local carrier_summary=""
    while IFS= read -r ca_line; do
        [ -z "$ca_line" ] && continue
        local ca_idx=$(echo "$ca_line" | awk -F',' '{print $1}' | tr -d ' ')
        local ca_band=$(echo "$ca_line" | awk -F',' '{print $2}' | tr -d ' ')
        local ca_earfcn=$(echo "$ca_line" | awk -F',' '{print $7}' | tr -d ' ')
        local ca_fields=$(echo "$ca_line" | awk -F',' '{print NF}')
        local ca_pci=""
        [ "$ca_fields" -ge 14 ] 2>/dev/null && ca_pci=$(echo "$ca_line" | awk -F',' '{print $7}' | tr -d ' ')
        local ca_rsrp=$(echo "$ca_line" | awk -F',' '{print $9}' | tr -d ' ')
        local ca_rsrq=$(echo "$ca_line" | awk -F',' '{print $10}' | tr -d ' ')
        local ca_dl_earfcn=$(echo "$ca_line" | awk -F',' '{print $11}' | tr -d ' ')
        local ca_dl_bw=$(_xmm_bw_mhz "$(echo "$ca_line" | awk -F',' '{print $13}' | tr -d ' ')")
        [ -z "$ca_dl_earfcn" ] && ca_dl_earfcn="$ca_earfcn"
        [ -z "$ca_band" ] && continue
...
EOF
    ...
    class="Cell Information"
    set_4g_cell_info "$mcc" "$mnc" "$tac" "$cellid" "$earfcn" "$pci" "$band" "$ul_bw" "$dl_bw" "$rsrp" "$rsrq" "$sinr" "" "" "Network Information"

    local rssi=""
    # RSSI from 3GPP relation RSRQ = 10log10(N_RB) + RSRP - RSSI  =>
    # RSSI = RSRP - RSRQ + 10log10(N_RB). XMM CSQ tracks RSRP (gives RSSI==RSRP),
    # so derive it properly from RSRP/RSRQ and the DL RB count; fallback to CSQ.
    local n_rb=""
    case "$dl_bw" in
        "1.4") n_rb=6 ;; "3") n_rb=15 ;; "5") n_rb=25 ;;
        "10") n_rb=50 ;; "15") n_rb=75 ;; "20") n_rb=100 ;;
