...
XMODEM_UBUS_OBJECT="xmodem"
XMODEM_UBUS_METHOD="save_stats"
...
save_usage_stats()
{
    ...
    response=$(ubus call "$XMODEM_UBUS_OBJECT" "$XMODEM_UBUS_METHOD" "$payload" 2>/dev/null) || {
        logger -t xmodem_usage_stats "ubus call failed for $config_section"
        return 1
    }

    status=$(jsonfilter -s "$response" -e '@.result.status' 2>/dev/null)
    if [ "$status" != "1" ]; then
        logger -t xmodem_usage_stats "failed to persist usage stats for $config_section"
        return 1
    fi
}
