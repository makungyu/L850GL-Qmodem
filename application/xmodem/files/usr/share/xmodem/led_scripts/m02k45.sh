...
. /usr/share/xmodem/led_scripts/connectivity.sh
. /lib/functions.sh
...
internet_led() {
	if xmodem_connectivity_probe 3; then
...
get_rsrp() {
	rsrp=$(/usr/share/xmodem/modem_ctrl.sh cell_info "$MODEM_CFG" | jq -r '.modem_info[] | select(.key=="RSRP") | .value')
