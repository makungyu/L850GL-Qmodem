...
sim_inserted() {
	sim_status=$(ubus call xmodem sim_info "{\"config_section\":\"$MODEM_CFG\"}" | jq -r '.modem_info[] | select(.key=="SIM Status") | .value')
...
get_mode() {
	local network_mode=$(ubus call xmodem cell_info "{\"config_section\":\"$MODEM_CFG\"}" | jq -r '.modem_info[] | select(.key=="network_mode") | .value')
...
get_rsrq() {
	rsrq=$(ubus call xmodem cell_info "{\"config_section\":\"$MODEM_CFG\"}" | jq -r '.modem_info[] | select(.key=="RSRQ") | .value')
