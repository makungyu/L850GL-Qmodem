...
method=$1
config_section=$2
at_port=$(uci -q get xmodem.$config_section.at_port)
override_at_port=$(uci -q get xmodem.$config_section.override_at_port)
[ -n "$override_at_port" ] && at_port=$override_at_port
uci -q get xmodem.$config_section.sms_at_port >/dev/null && sms_at_port=$(uci -q get xmodem.$config_section.sms_at_port)
vendor=$(uci -q get xmodem.$config_section.manufacturer)
platform=$(uci -q get xmodem.$config_section.platform)
pdp_index=$(uci -q get xmodem.$config_section.pdp_index)
[ -z "$pdp_index" ] && pdp_index=$(uci -q get xmodem.$config_section.suggest_pdp_index)
use_ubus=$(uci -q get xmodem.$config_section.use_ubus)
modem_path=$(uci -q get xmodem.$config_section.path)
...
vendor_script_prefix="/usr/share/xmodem/vendor"
...
get_at_cfg(){
    ...
    ports=$(uci -q get xmodem.$config_section.ports)
    for port in $ports; do
        json_add_string "" "$port"
    done
    json_close_array
    json_add_array valid_ports
    v_ports=$(uci -q get xmodem.$config_section.valid_at_ports)
    ...
    case "$lang" in
        zh*|cn|auto)
            at_commands_file="/usr/share/xmodem/at_commands_zh.json"
            ;;
        *)
            at_commands_file="/usr/share/xmodem/at_commands_en.json"
            ;;
    esac
    
    # Fallback to default file if language-specific file doesn't exist
    [ ! -f "$at_commands_file" ] && at_commands_file="/usr/share/xmodem/at_commands.json"
...
case $method in
    ...
    "clear_dial_log")
        json_select result
        log_file="/var/run/xmodem/${config_section}_dir/dial_log"
